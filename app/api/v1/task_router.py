...
from app.schemas.task_schema import TaskPage, TaskSchema, TaskCreate, TaskUpdate, TaskResponse
...
async def list_paginated(
    ...
    task_output = TaskPage(
        data=[TaskSchema.from_orm(t) for t in tasks],
        total=total,
        page=page,
        page_size=size,
...
async def get_task(task_id: int):
    ...
    if not task:
        logger.error("get_task_failed", reason="not_found")
...
async def update(task_id: int, data: TaskUpdate):
    ...
    if not task:
        logger.error("update_task_failed", reason="not_found")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Task not found!")
...
async def delete(task_id: int):
    ...
    if not success:
        logger.error("delete_task_failed", reason="not_found")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Task not found!")
